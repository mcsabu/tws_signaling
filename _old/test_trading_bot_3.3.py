...
def getData4(contracts4, defaultid, scan4):
    for i in contracts4:
        ...
        time.sleep(2.5)  # sleep to allow enough time for data to be returned
...
def prepare_data_into_individual_frames3(df0):
    dfs.clear()
    df0['Date'] = pandas.to_datetime(df0['Date']) # ez lehet nem kell a md_listhez - datetime...
    ...
    for j in df.index:
        try:
            if (j > red_bar) & (df.OnlyTime[j] < tme):
                if (df.High[j] > red_bar_top) & (df.Close[j] > 0.75) & (j - red_bar < 9):
                    if df.index[-1] == j:
                        alert = True
...
def md_multiday_breakout_RTH(df):
    a = 0
    #5 day breakout alert; If high is higher than before 5 day high, only RTH

def md_intraday_breakout(df):
