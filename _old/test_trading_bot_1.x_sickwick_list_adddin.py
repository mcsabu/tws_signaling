...
def Sick_wick(df):
    ...
    for i in df.index:
        try:
            if (df.Volume[i] > 20000) & (df.Close[i] > 0.75) \
                & (df.High[i] / df.Low[i] > 1.10) & (df.Close[i] > df.Open[i]):
                for j in range(i-30*78, i):
                    top_volume_bar = 0
                    if df.Volume[j] > top_volume_bar:
                        top_volume_bar = df.Volume[j]
                        top_bar = j
                        top_bar_low = df.Low[j]
                    if j == i-1:
                        if df.Close[i] < top_bar_low:
    ...
    for i in range(len(update_timing_list)):
        ...
        while current_time < update_timing_list[i]:
            ...
            scan2 = pandas.read_csv(
                r'C:\Users\user\PycharmProjects\pythonProject\sick_wick_scanner_list.csv')
            contracts2 = scan2['Symbol'].tolist()
            getData2(contracts, defaultid, scan)
            app.data = []
            print('updating_data')
            print(datetime.now().strftime('%H:%M:%S'))
            df0 = read_csv(r'C:\Users\user\PycharmProjects\pythonProject\trading_list_datas.csv')
