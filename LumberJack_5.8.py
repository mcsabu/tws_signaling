...
update_timing_list = []
defaultid = 0
alerted_list = [['', '', datetime.now() + timedelta(hours=10)]]
date_plot, close_plot, open_plot, high_plot, low_plot, volume_plot, vwap_plot, fig_plot = [], [], [], [], [], [], [], []
red_bar_time_list = []
compressed_time_list = []

starting_time = dtime(hour=0, minute=0, second=0)
red_bar_time_list.append(starting_time)
compressed_time_list.append(starting_time)
                ...
                if (df.Volume[j] > 10000) & (df.Close[j] > 0.75) \
                    ...
                    if df.Close[j] < top_bar_low:
                        if df.index[-1] == j:
                            ...
                            alerted = [df['Symbol'].iloc[-1], strategy, datetime.now()]
        except KeyError:
            pass
    if alert == True:
        for any_list in alerted_list:
            if (any_list[0] == alerted[0]) & (any_list[1] == alerted[1]):
                alert = False
...
def Green_climb_prem_short(df):
    ...
    if alert == True:
        for any_list in alerted_list:
            if (any_list[0] == alerted[0]) & ((any_list[1] == alerted[1]) | (any_list[1] == 'Green_bounce_inclusive') | (any_list[1] == 'Green_bounce')):
                alert = False
    if (alert == True):
...
def Cons_breakout(df):
    ...
    df = df.groupby(['OnlyDate', 'Symbol'], group_keys=False, as_index=False).apply(VWAP)
    #df = df.reset_index()
    def SMA(df):
        return df.assign(SMA=df.Close.rolling(window=10).mean())

    df = df.groupby(['OnlyDate', 'Symbol'], group_keys=False, as_index=False).apply(SMA)
    #df = df.reset_index()
    def avg_volume(df):
        return df.assign(avg_volume=df.Volume.rolling(window=5).mean())

    df = df.groupby(['OnlyDate', 'Symbol'], group_keys=False, as_index=False).apply(avg_volume)
    #df = df.reset_index()
    def stdev(df):
        return df.assign(stdev=df.Close.rolling(window=10).std())

    df = df.groupby(['OnlyDate', 'Symbol'], group_keys=False, as_index=False).apply(stdev)
    #df = df.reset_index()
    def prev_min(df):
        return df.assign(prev_min=df.Low.rolling(window=5).min())

    df = df.groupby(['OnlyDate', 'Symbol'], group_keys=False, as_index=False).apply(prev_min)
    #df = df.reset_index()
    def prev_min_far(df):
        return df.assign(prev_min_far=df.Low.rolling(window=20).min())

    df = df.groupby(['OnlyDate', 'Symbol'], group_keys=False, as_index=False).apply(prev_min_far)
    #df = df.reset_index()
    def prev_max(df):
        return df.assign(prev_max=df.High.rolling(window=10).max())

    df = df.groupby(['OnlyDate', 'Symbol'], group_keys=False, as_index=False).apply(prev_max)
    ...
    df = df.groupby(['OnlyDate', 'Symbol'], group_keys=False, as_index=False).apply(Bollinger_max)
...
def Green_bounce_inclusive(df):
    ...
    if alert == True:
        for any_list in alerted_list:
            if (any_list[0] == alerted[0]) & ((any_list[1] == alerted[1]) | (any_list[1] == 'Green_climb_prem_short') | (any_list[1] == 'Green_bounce')):
                alert = False
    if (alert == True):
...
def Green_bounce(df):
    ...
    if alert == True:
        for any_list in alerted_list:
            if (any_list[0] == alerted[0]) & ((any_list[1] == alerted[1]) | (any_list[1] == 'Green_climb_prem_short') | (any_list[1] == 'Green_bounce_inclusive')):
                alert = False
    if (alert == True):
...
def Jump_fail(df):
    ...
    if df.Close.iloc[-1] > df.Open.iloc[-1]:
        entryPrice = df.Close.iloc[-1]
    else:
        entryPrice = df.Low.iloc[-1]

    entryVolume = df.Volume.iloc[-1]
    maxPrice = max(df.High.iloc[-2], df.High.iloc[-3], df.High.iloc[-4], df.High.iloc[-5], df.High.iloc[-6])
    maxVolume = max(df.Volume.iloc[-2], df.Volume.iloc[-3], df.Volume.iloc[-4], df.Volume.iloc[-5], df.Volume.iloc[-6])
    startPrice = min(df.Low.iloc[-2], df.Low.iloc[-3], df.Low.iloc[-4], df.Low.iloc[-5], df.Low.iloc[-6])
    minVolume = min(df.Volume.iloc[-2], df.Volume.iloc[-3], df.Volume.iloc[-4], df.Volume.iloc[-5], df.Volume.iloc[-6])

    if minVolume == 0:
        minVolume = 1

    if (maxPrice / startPrice > 1.20) & (entryPrice > 0.75):
        if (maxVolume / entryVolume > 1.5) & (maxVolume / minVolume > 10) & (maxVolume > 100000):
            minPrice2 = 100000
            startPriceIndex = 0
            maxPrice2 = 0
            maxPriceIndex = 0
            for i in range(df.index[-6], df.index[-1]+1):
                if (df.Low[i] < minPrice2) & (df.Volume[i] > 5000):
                    minPrice2 = df.Low[i]
                    startPriceIndex = i
                if df.High[i] > maxPrice2:
                    maxPrice2 = df.High[i]
                    maxPriceIndex = i
                if i == df.index[-1]:
                    if maxPrice2 / minPrice2 > 1.2:
                        if (maxPrice2 - entryPrice) / (maxPrice2 - minPrice2) > 0.45:
                            if maxPriceIndex > startPriceIndex:
                                prevVolume = 0
                                for j in range(df.index[-32], df.index[-6]):
                                    if df.Volume[j] > prevVolume:
                                        prevVolume = df.Volume[j]
                                    if j == df.index[-7]:
                                        if maxVolume / prevVolume > 5:
                                            alert = True
                                            strategy = 'Jumpfail'
                                            alerted = [df['Symbol'].iloc[-1], strategy, datetime.now()]
...
def Jump_fail_RTH(df):
    ...
    if alert == True:
        for any_list in alerted_list:
            if (any_list[0] == alerted[0]) & ((any_list[1] == alerted[1]) | (any_list[1] == 'Jumpfail_prem') | (any_list[1] == 'Jumpfail_prem2_RTH_afterm')):
                alert = False
    if (alert == True):
...
def red_bar_reclaim(df):
    ...
    if (alert == True):
        ...
        buy_signal_list.append(df.iloc[-1])
        make_plot(date=df.Date, close=df.Close, open=df.Open, high=df.High, low=df.Low,
                  volume=df.Volume, idx=df.index[-1], symbol=df.Symbol.iloc[-1], prev=25)
        send_email(strategy, df['Symbol'].iloc[-1])
        alerted_list.append(alerted)
...
def Jump_fail_prem(df):
    ...
    if alert == True:
        for any_list in alerted_list:
            if (any_list[0] == alerted[0]) & ((any_list[1] == alerted[1]) | (any_list[1] == 'Jumpfail_RTH') | (any_list[1] == 'Jumpfail_prem2_RTH_afterm')):
                alert = False
    if (alert == True):
...
def Jumpfail_prem2_RTH_afterm(df):
    ...
    if alert == True:
        for any_list in alerted_list:
            if (any_list[0] == alerted[0]) & ((any_list[1] == alerted[1]) | (any_list[1] == 'Jumpfail_RTH') | (any_list[1] == 'Jumpfail_RTH')):
                alert = False
    if (alert == True):
...
def Sickle(df):
    ...
    if (alert == True) & (df.Symbol.iloc[-1] not in end_day_list):
        end_day_list.append(df.Symbol.iloc[-1])
        print('Sickle')
        buy_signal_list.append(df.iloc[-1])
        ...
        try:
            if (df.Volume[i] > 100000) & (df.Close[i] > 1):
                ...
                for j in range(i - 15, i):
                    if (top_volume < df.Volume[j]) & (df.OnlyDate[i] == df.OnlyDate[j]):
                        top_volume = df.Volume[j]
                        next_bar_volume = df.Volume[j + 1]
                        next2_bar_volume = df.Volume[j + 2]

                if (top_volume > 500000) & (next_bar_volume > 250000) & (next2_bar_volume > 250000):
...
def running():
    ...
    for i in range(len(update_timing_list)):
        ...
        while current_time < update_timing_list[i]:
            ...
            for list in alerted_list:
                if list[2] < datetime.now() - timedelta(minutes=45):
                    alerted_list.remove(list)
